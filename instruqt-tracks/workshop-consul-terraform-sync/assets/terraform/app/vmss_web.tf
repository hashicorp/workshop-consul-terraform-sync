...
resource "azurerm_virtual_machine_scale_set" "web_vmss" {
  ...
  os_profile {
    ...
    custom_data          = base64encode(templatefile("./templates/web_server.sh", { endpoint = var.endpoint, consulconfig = var.consulconfig, ca_cert = var.ca_cert, consul_token = var.consul_token, bigip_mgmt_addr = var.bigip_mgmt_addr }))
  }
